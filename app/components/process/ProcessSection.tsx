    ...
    >
      <div className="mx-auto w-full max-w-7xl">
        <div className="bg-white/10 backdrop-blur-md rounded-[25px] p-6 sm:p-8 md:p-12 border border-white/10 w-full overflow-hidden">
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white text-center mb-8 sm:mb-12">{process?.title ?? 'Our Process'}</h2>
          ...
          >
            {steps.length === 0 ? (
              <div className="w-full text-center py-8 text-white">No steps available</div>
            ) : (
              steps.map((step) => (
                <div key={step.id} className="process-card w-full sm:w-[calc(50%-0.5rem)] md:w-[calc(33.333%-0.75rem)] lg:w-[calc(25%-0.75rem)] xl:w-[calc(20%-0.8rem)]">
                  <StepCard
                    stepId={step.id}
                    title={step.title ?? ""}
